...
class componentVisual(wx.Frame):
    ...
    def on_timer(self):
        self.value.SetLabel(self._value())
        self.Refresh()
        self.Update()
        self.timer.Restart()
    ...
    def _set_display(self):
        ...
        value_label = wx.StaticText(self, label="Valor actual: ")
        value_sizer.Add(
            value_label,
            0,
        ...
        self.value = wx.StaticText(
            self, label=self._value(), style=wx.ALIGN_RIGHT
        )
        self._set_up_display(self.value, value_sizer)
        self.sizer.Add(value_sizer, 0, wx.EXPAND)

    @staticmethod
    def _set_up_display(display_text, sizer, foreground_color=None, maximum_digits=10):
        if foreground_color is None:
            foreground_color = componentVisual._silver()
        font = wx.Font(
            18, wx.FONTFAMILY_TELETYPE, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL
        )
        display_text.SetFont(font)
        display_text.SetBackgroundColour(componentVisual._black())
    ...
    def _value(self):
        return str(self.simple_test_component.get_status()[0])
